(function ($) {
    ...
    $(function() {
        sliderrange.slider({
            ...
            slide: function(event, ui) {
                ...
                $('#price-min').val(ui.values[0]); // Update hidden inputs
                $('#price-max').val(ui.values[1]); // Update hidden inputs
            }
        });
   ...
   /*---------------------
    ...
    $('.nav-link').on('click', function() {
        ...
        $(target).addClass('show active'); // Show the clicked content
    });
    ...
    $(document).ready(function() {
        ...
        $('.add-to-wishlist').click(function(e) {
            ...
            var isActive = $this.find('.fi-rs-heart').hasClass('active');
    
            $.ajax({
                url: isActive ? '/wishlist/remove/' + productId : '/wishlist/add/' + productId,
                type: isActive ? 'DELETE' : 'POST',
                success: function(response) {
                    if (response.success) {
                        // Toggle the heart icon state
                        $this.find('.fi-rs-heart').toggleClass('active');
                        Swal.fire({
                            icon: 'success',
                            title: isActive ? 'Removed from Wishlist!' : 'Added to Wishlist!',
                            text: isActive ? 'The product has been removed from your wishlist.' : 'The product has been added to your wishlist.',
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: isActive ? 'Could not remove from wishlist. Please try again.' : 'Could not add to wishlist. Please try again.',
                        });
                    }
                },
                error: function() {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: 'Error in updating wishlist.',
                    });
                }
            });
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        swal.fire('Error', 'Could not remove product from wishlist', 'error');
                    }
                },
                error: function() {
