...
export const getCategories = asyncHandler(async (req, res) => {
  ...
  const categories = await Category.find().skip(startIndex).limit(limit).sort({ createdAt: -1 });
  // res.json({
  //     status:'success',
  //     msg:'All categories fetched successfully',
  //     categories
  // })
  return res.render("addCategories", { categories, admin, currentPage: page,
    totalPages: Math.ceil(total / limit),
    limit,   error: req.query.error || null,
...
export const createCategory = asyncHandler(async (req, res) => {
  try {
    ...
    // Validate input
    if (!name || !req.file) {
      return res.status(400).json({ status: 'error', message: 'Name and Image are required to create a category.' });
    }
