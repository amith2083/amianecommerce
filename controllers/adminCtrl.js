...
import { getTopSellingProducts,getTopSellingCategories } from '../utils/bestseller.js';
...
//    }
   ...
   export const adminDashboard = async(req,res)=>{
    try{
        ...
        const orders = await Order.find({ paymentStatus: 'Paid'} ).populate('user').populate('orderItems.productId'); 
        ...
        const revenue = await Order.aggregate([
          ...
          for (let i = 11; i >= 0; i--) {
            ...
            const salesRecord = monthlySalesData.find(record => record._id === monthNumber);
...
res.render('adminDashboard',{admin,orders,categories,revenue: revenue[0]?.totalRevenue || 0,
...
export const bestSelling = asyncHandler(async (req, res) => {
  try {
    ...
    res.render('bestSelling', {  bestSellingProducts ,bestSellingCategories,admin });
  } catch (error) {
    console.error(error);
...
export const generateSalesReport = async (req, res) => {
    try {
      ...
      // }
       ...
       const ordersWithDiscount = await Promise.all(orders.map(async (order) => {
